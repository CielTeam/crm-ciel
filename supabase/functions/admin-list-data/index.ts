...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      await sb.from('audit_logs').insert({ actor_id: 'anonymous', action: 'auth.failure', target_type: 'admin-list-data', metadata: { reason: err instanceof Error ? err.message : 'Unknown' } });
    } catch { /* best effort */ }
  ...
  if (!checkRateLimit(`admin-list:${actorId}`, 30, 60_000)) {
