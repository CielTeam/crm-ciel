...
// Pinned countries appear first in selectors; the rest follow alphabetically.
...
export const PINNED_COUNTRY_CODES = ['LB', 'AE', 'SA', 'QA', 'KW', 'OM', 'BH', 'JO', 'EG', 'TR', 'FR', 'GB'] as const;
...
export const COUNTRIES: Country[] = [
  ...
  { code: 'GD', name: 'Grenada' }, { code: 'GU', name: 'Guam' }, { code: 'GT', name: 'Guatemala' },
...
const COUNTRY_BY_CODE: Record<string, Country> = COUNTRIES.reduce((acc, c) => {
  acc[c.code] = c;
  return acc;
...
export function getCountryName(code: string | null | undefined, fallback?: string | null): string {
  return getCountryByCode(code)?.name || fallback || '';
}

export function isValidCountryCode(code: unknown): code is string {
  return typeof code === 'string' && code.length === 2 && !!COUNTRY_BY_CODE[code.toUpperCase()];
}

/** Pinned (in pin order) followed by alphabetical. */
export function getOrderedCountries(): { pinned: Country[]; rest: Country[] } {
