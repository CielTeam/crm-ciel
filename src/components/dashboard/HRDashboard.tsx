...
export function HRDashboard({ stats }: { stats: DashboardStats }) {
  ...
  const personalStats = [
    { label: 'My Open Tasks', value: stats.openTasks ?? 0, icon: CheckSquare, iconColor: 'text-success' },
    { label: 'My Pending Leaves', value: stats.pendingLeaves ?? 0, icon: Palmtree, iconColor: 'text-warning' },
    { label: 'Unread Notifications', value: stats.unreadMessages ?? 0, icon: Bell, iconColor: 'text-primary' },
    { label: 'Upcoming Meetings', value: '—', icon: Calendar, iconColor: 'text-info' },
  ];

  const leaveByType = stats.leaveByType ?? {};
  const pendingLeaves = (stats.pendingLeaveList ?? []) as PendingLeave[];
            ...
            {pendingLeaves.length === 0 ? (
              ...
              pendingLeaves.map((l) => (
                ...
                >
                  ...
                  <Badge
                    variant="outline"
                    className="bg-warning/15 text-warning text-[10px] shrink-0"
                  >
                    Pending
                  </Badge>
                </div>
