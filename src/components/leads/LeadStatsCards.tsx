...
export function LeadStatsCards() {
  ...
  const row2: StatCard[] = [
    { label: 'Pipeline Value', value: stats?.pipeline_value ? `$${Number(stats.pipeline_value).toLocaleString()}` : '$0', icon: DollarSign, color: 'text-primary' },
    { label: 'Weighted Forecast', value: stats?.weighted_forecast ? `$${Number(stats.weighted_forecast).toLocaleString()}` : '$0', icon: TrendingUp, color: 'text-[hsl(var(--success))]' },
