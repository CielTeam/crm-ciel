...
export function AddLeadDialog({ open, onOpenChange }: Props) {
  ...
  const handleSubmit = () => {
    ...
    create.mutate(form, { onSuccess: () => { onOpenChange(false); setForm({ company_name: '', contact_name: '', contact_email: '', contact_phone: '', status: 'potential', source: '', notes: '' }); } });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <DialogFooter><Button onClick={handleSubmit} disabled={create.isPending || !form.company_name.trim() || !form.contact_name.trim()}>{create.isPending ? 'Creating...' : 'Create Lead'}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
